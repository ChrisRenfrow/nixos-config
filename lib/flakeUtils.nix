...
let
  ...
  mapDir = mapper: path:
    mapAttrs (n: _: let p = path + "/${n}"; in mapper p)
    (optionalAttrs (pathExists path) (readDir path));

  mapAllFiles = mapper: path:
    flatten (mapAttrsToList (n: v:
      let p = path + "/${n}";
      in if v == "directory" then mapAllFiles mapper p else mapper p)
      (optionalAttrs (pathExists path) (readDir path)));
  ...
  mkHost = path:
    attrs@{ modulesPath, pkgsLocal, home-manager, localLib, inputs, ... }:
...
in { inherit collectHosts collectModules collectPackages; }
